        ...
        Papa.parse("/demand_inventory.csv", {
            ...
            complete: (result) => {
                ...
                filtered.forEach((r) => {
                    ...
                    if (!monthly_units_sold[ym]) {
                        monthly_units_sold[ym] = {units_sold: 0};
                    }

                    monthly_units_sold[ym].units_sold += Number(r.units_sold) || 0;
                });

                const sortedMonths = Object.keys(monthly_units_sold)
                                           .filter((m) => m >= "2023-01")
                                           .sort();

                const labels = sortedMonths;

                const units_sold = sortedMonths.map((m) => monthly_units_sold[m].units_sold);
