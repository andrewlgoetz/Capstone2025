...
const Landing = () => {
  ...
  const handleScan = (code) => {
    ...
    if (item) {
      // For now, just highlight the item in the table
      setQuery(code); // This will filter the table to show just the scanned item
    } else {
            ...
            <div className={styles.fabInner}>
              {showFABMenu && (
                <div className={styles.fabMenu}>
                  <FABItem label="Scan" onClick={() => { setShowScan(true); setShowFABMenu(false); }} />
                  <FABItem label="Update inventory" onClick={() => setShowFABMenu(false)} />
                  <FABItem label="...Edit" onClick={() => setShowFABMenu(false)} />
                </div>
              )}
...
function FABItem({ label, onClick }) {
  return (
    <button className={styles.btnGhost} onClick={onClick}>{label}</button>
  )
}
